'use strict';
/*
 * SBC-NG wsbridge (portado de PBX-NG) — cliente SIP-over-WSS (RFC 7118) hacia troncales WebRTC remotas.
 ...
 *   Gateado por sbc_settings.mod_wsbridge. Troncales sbc_trunks mode='webrtc-client'.
 *   El relay va a Kamailio LOCAL (127.0.0.1:5060), que rutea a la central.
 */
...
const DB = {
  ...
  database: process.env.DB_NAME || 'sbcng', user: process.env.DB_USER || 'sbcng', password: process.env.DB_PASS || '',
};
// El puente entrega las llamadas al Kamailio LOCAL del borde; Kamailio las rutea.
const AST_HOST = process.env.KAM_HOST || '127.0.0.1';
...
class TrunkClient {
  constructor(t) {
    ...
    this.authTried = false; this.authFails = 0;  // #123: cortar el loop de REGISTER con credenciales malas
  }
  ...
  connect() {
    ...
    this.ws.on('open', () => { this.setState('connected', 'WSS abierto'); this.authTried = false; this.register(); });
  ...
  register(auth) {
    ...
    const L = [`REGISTER ${uri} SIP/2.0`, `Via: SIP/2.0/WSS ${this.localVia};branch=${branch};rport`, `Max-Forwards: 70`,
      ...
      `CSeq: ${this.cseq} REGISTER`, `Contact: <sip:${this.user}@${this.localVia};transport=ws>;expires=600`, `Expires: 600`, `User-Agent: SBC-NG-wsbridge`];
    if (auth) L.push(auth);
  ...
  onRegisterResp(p) {
    if (p.status === 401 || p.status === 407) {
      // Segundo 401 seguido = ya mandamos el digest y el servidor lo rechazo:
      // las credenciales estan mal. Reintentar al instante seria un loop cerrado
      // de REGISTER/401 que martilla la central. Se hace backoff y no se reintenta
      // hasta dentro de un rato (crece con cada fallo, tope 30 min).
      if (this.authTried) return this.authBackoff('credenciales rechazadas');
      this.authTried = true;
    ...
    if (p.status >= 200 && p.status < 300) {
      this.authTried = false; this.authFails = 0;  // registro OK: se limpia el backoff
      this.setState('online', 'Registrada (WSS)'); this.cseq++;
      clearTimeout(this.regTimer); this.regTimer = setTimeout(() => { this.authTried = false; this.register(); }, 270000); return;
    }
    // 403/404/otros: rechazo duro (cuenta bloqueada, sin permiso). Tambien con backoff.
    if (p.status === 403 || p.status === 404) return this.authBackoff('registro rechazado ' + p.status);
    this.setState('offline', 'Registro rechazado ' + p.status);
  }

  // Backoff tras fallo de autenticacion/rechazo: 60s, 120s, 240s ... tope 30 min.
  // Evita el loop de REGISTER y da tiempo a corregir la credencial en el panel.
  authBackoff(motivo) {
    clearTimeout(this.regTimer);
    const espera = Math.min(60000 * Math.pow(2, this.authFails), 1800000);
    this.authFails++;
    this.setState('error', `${motivo}; reintento en ${Math.round(espera / 1000)}s`);
    this.regTimer = setTimeout(() => { this.authTried = false; this.cseq++; this.register(); }, espera);
  }
  ...
  async onInboundInvite(p, msg) {
    ...
    const target = userOf(p.ruri) || p.ruri;
    ...
    const ctx = {
      aCallId: callId, bCallId, a: this, p, fromTag, bFromTag, bBranch, target, bToTag: null,
    ...
    const inv = [`INVITE sip:${target}@${AST_HOST} SIP/2.0`, `Via: SIP/2.0/UDP ${LOCAL_IP}:${LOCAL_SIP_PORT};branch=${bBranch};rport`, `Max-Forwards: 70`,
      ...
      `CSeq: 1 INVITE`, `Contact: <sip:${this.user}@${LOCAL_IP}:${LOCAL_SIP_PORT}>`, `Content-Type: application/sdp`, `User-Agent: SBC-NG-wsbridge`,
      `Content-Length: ${Buffer.byteLength(off.sdp)}`, '', off.sdp].join('\r\n');
    log(`[${this.name}] offer ${off.result}; INVITE -> Asterisk ${target}`);
    this._astSend(inv);
  }
  async _astToRemote(ctx, bp) {
    if (!bp.status) return;
    log(`[${this.name}] Asterisk resp ${bp.status}`);
...
async function reconcile(db) {
  ...
  try {
    const m = await db.query("SELECT value FROM sbc_settings WHERE key='mod_wsbridge'"); modOn = m.rows[0] ? m.rows[0].value !== '0' : true;
    const r = await db.query("SELECT name, remote_url, username, password FROM sbc_trunks WHERE mode='webrtc-client' AND enabled");
    trunks = r.rows.map((x) => ({ name: x.name, remote_url: x.remote_url, username: x.username, password: x.password })).filter((x) => x.remote_url && x.username);
  ...
  try {
    ...
    for (const [name, c] of clients) await db.query("INSERT INTO sbc_wsbridge_status (name,state,detail,updated_at) VALUES ($1,$2,$3,now()) ON CONFLICT (name) DO UPDATE SET state=$2,detail=$3,updated_at=now()", [name, c.state, c.detail]);
    if (!modOn) await db.query("UPDATE sbc_wsbridge_status SET state='disabled', detail='módulo apagado', updated_at=now()");
  } catch (_) {}
}
