'use client';
/* ============================================================================
 *  Respaldo y restauración de SBC-NG.
 *
 *  Diferencia importante con la central: acá restaurar NO reinicia Kamailio.
 *  Deja los fragmentos en su lugar y te manda a "Aplicar", que es el camino que
 *  ya valida en un Kamailio efímero y revierte solo si el motor no levanta.
 *  Restaurar y aplicar son dos decisiones distintas.
 * ==========================================================================*/
...
import {
  ...
  IconRestore, IconShieldLock, IconClock, IconFileZip, IconCheck, IconInfoCircle, IconBolt,
...
export default function Respaldos() {
  ...
  const [conGeo, setConGeo] = useState(false);
  ...
  const crear = async () => {
    ...
    try {
      await toastPromise(
        api('/backup', { method: 'POST', body: { geoip: conGeo, nota } }).then((r) => { cargar(); return r; }),
        { loading: 'Creando respaldo…', success: (r) => `Respaldo creado · ${peso(r.bytes)}`,
  ...
  const subir = async (file) => {
    if (!file) return;
    if (!file.name.endsWith('.tar.gz')) { toast('Tiene que ser un .tar.gz de SBC-NG', 'bad'); return; }
    setSubiendo(true);
    try {
      await toastPromise(
        fetch('/backend/api/v1/backup/subir/' + encodeURIComponent(file.name), {
  ...
  const abrirRestaurar = async (nombre) => {
    try { setRest({ nombre, m: await api(`/backup/${encodeURIComponent(nombre)}/inspeccionar`) }); setPalabra(''); }
  ...
  const restaurar = async () => {
    ...
    try {
      await toastPromise(
        ...
        { loading: 'Restaurando…',
          success: (r) => `Restaurado: ${r.restauradas.join(', ')}. Ahora aplicá desde Motor SIP.`,
  ...
  return (
    <Stack gap="md" className="sbc-fade-in">
      <Group gap={10}>
        <ThemeIcon size={34} radius="md" variant="light" color="sbc"><IconDatabaseExport size={19} /></ThemeIcon>
        <div>
          <Text fw={700}>Respaldo y restauración</Text>
          <Text size="xs" c="dimmed">Guardá la configuración del borde en un archivo, y volvé a levantarla desde él.</Text>
        </div>
      </Group>

      <Alert color="orange" variant="light" icon={<IconShieldLock size={18} />} title="Tratá el archivo como material sensible">
        <b>Incluye la clave privada de firma de STIR/SHAKEN</b> y el token de métricas. Si no las
        llevara, un borde restaurado dejaría de firmar y habría que rehacer el alta con el operador.
        Guardalo como guardarías una llave.
        <br />
        No incluye las credenciales del entorno (base de datos, secreto de sesiones): la
        instalación donde lo restaures usa <b>las suyas</b>.
      </Alert>
      ...
      <Card withBorder radius="md" p="md">
        <Group justify="space-between" align="flex-end" wrap="wrap" gap="md">
          <Stack gap={8} style={{ flex: 1, minWidth: 280 }}>
            ...
            <TextInput size="xs" placeholder="Nota (opcional): 'antes de tocar el ruteo'"
              value={nota} onChange={(e) => setNota(e.currentTarget.value)} maxLength={300} />
            <Checkbox size="xs" checked={conGeo} onChange={(e) => setConGeo(e.currentTarget.checked)}
              label="Incluir la base de geolocalización"
              description="Pesa varios MB y se puede volver a bajar. Sirve para armar una instalación que no vaya a tener internet." />
          </Stack>
          <Group gap="sm">
            ...
            <Button leftSection={<IconDatabaseExport size={16} />} loading={creando} onClick={crear} color="sbc">Crear respaldo</Button>
          </Group>
        </Group>
      </Card>
        ...
        {!data ? <Group justify="center" p="xl"><Loader size="sm" /></Group>
          : lista.length === 0 ? (
            <Stack align="center" gap={6} p="xl">
              ...
              <Text fz="xs" c="dimmed" ta="center" maw={430}>
                Conviene crear uno ahora y otro antes de cada cambio de ruteo o de seguridad. Un
                respaldo que nunca se probó no es un respaldo: bajalo y guardalo fuera del borde.
              </Text>
            </Stack>
          ) : (
              ...
              <Table.Tbody>
                {lista.map((b) => (
                  <Table.Tr key={b.nombre}>
                    <Table.Td><Group gap={8}><ThemeIcon size={26} radius="sm" variant="light" color="sbc"><IconFileZip size={14} /></ThemeIcon>
                    ...
                    <Table.Td><Group gap={4} justify="flex-end" wrap="nowrap">
                      <Tooltip label="Descargar"><ActionIcon variant="subtle" component="a"
                        href={`/backend/api/v1/backup/${encodeURIComponent(b.nombre)}/archivo`}><IconDownload size={16} /></ActionIcon></Tooltip>
      ...
      <Modal opened={!!rest} onClose={() => setRest(null)} size="lg" radius="md" centered
        title={<Group gap={8}><ThemeIcon size={30} radius="md" variant="light" color="orange"><IconRestore size={17} /></ThemeIcon>
          <div><Text fw={700}>Restaurar el borde</Text><Text fz="xs" c="dimmed">{rest && rest.nombre}</Text></div></Group>}>
        {rest && (
          <Stack gap="sm">
            {compat && !compat.ok ? (
              ...
              <>
                <Alert color="orange" variant="light" icon={<IconAlertTriangle size={18} />} title="Esto pisa la configuración actual">
                  Se reemplazan la base y los fragmentos de configuración del borde.
                  <b> El motor NO se reinicia solo</b>: después vas a Motor SIP → Aplicar, que valida
                  en un Kamailio aparte y revierte si no levanta. Antes de tocar nada se crea un
                  respaldo automático.
                </Alert>
                ...
                <Group justify="flex-end" gap="sm">
                  ...
                  <Button color="orange" leftSection={<IconBolt size={16} />} loading={restaurando}
