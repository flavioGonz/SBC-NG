...
# Logo SBC-NG (escudo + dos dialogos encadenados) en version estatica y monocroma,
# para la portada. El color lo define quien lo llama: blanco sobre el marco de acento,
# o el propio acento con opacidad baja para la marca de agua.
def logo_svg(size=30, stroke="#fff", opacity=1.0):
    return (
        f'<svg width="{size}" height="{size}" viewBox="0 0 64 64" fill="none" '
        f'style="opacity:{opacity}" xmlns="http://www.w3.org/2000/svg" aria-label="SBC-NG">'
        f'<path d="M32 4.5 L56 13.5 V31 c0 14.4 -9.9 25.5 -24 28.5 C17.9 56.5 8 45.4 8 31 V13.5 Z" '
        f'fill="none" stroke="{stroke}" stroke-width="3.4" stroke-linejoin="round"/>'
        f'<path d="M20 20 h13 a3.5 3.5 0 0 1 3.5 3.5 v9 a3.5 3.5 0 0 1 -3.5 3.5 h-6 l-5 5 v-5 h-2 '
        f'a3.5 3.5 0 0 1 -3.5 -3.5 v-9 A3.5 3.5 0 0 1 20 20 Z" fill="none" stroke="{stroke}" '
        f'stroke-width="3" stroke-linejoin="round"/>'
        f'<path d="M31 27.5 h13 a3.5 3.5 0 0 1 3.5 3.5 v9 A3.5 3.5 0 0 1 44 43.5 h-2 v5 l-5 -5 h-6 '
        f'a3.5 3.5 0 0 1 -3.5 -3.5 v-9 a3.5 3.5 0 0 1 3.5 -3.5 Z" fill="none" stroke="{stroke}" '
        f'stroke-width="3" stroke-linejoin="round" opacity="0.65"/>'
        f'</svg>'
    )
...
def md(text):
    ...
    while i < len(lines):
        ...
        if m:
            ...
            fname = src.split('/')[-1]
            FIG[0] += 1
            out.append(f'<figure class="shot"><div class="frame"><img src="/backend/api/v1/manuales/img/{fname}" alt="{inline(alt)}" '
...
.cover .wm{position:absolute;right:-40px;top:120px;opacity:.06;transform:rotate(-8deg);pointer-events:none}
...
@media print{
  ...
  @page{@bottom-right{content:"Pagina " counter(page) " / " counter(pages);font-size:9px;color:#94a3b8}}
  @page:first{margin:0}
  @page:first{@bottom-right{content:""}}
  /* La imagen siempre empieza arriba de la hoja: se lleva su titulo consigo y nada la parte. */
  figure.shot{break-inside:avoid;margin:20px 0 24px}
  figure.shot .frame{padding:10px}
  figure.shot img{max-height:118mm}
...
<div class="page">
  <header class="cover">
      <div class="wm">{logo_svg(280, manual['accent'])}</div>
    <div class="bar"></div><div class="side"></div>
    <div class="inner">
      <div class="brandline">
        <div class="mark">{logo_svg(30, "#fff")}</div>
        <div>
